...
def run_on_one_file_parallel(func_args: tuple):
    args, full_recording_file, full_header_file, original_output_dir = func_args
...
def run(args):
    ...
    for full_header_file, full_recording_file in zip(
        ...
        func_args.append(
            (deepcopy(args), full_recording_file, full_header_file, original_output_dir)
        )

    if args.num_cpus > 1:
        with multiprocessing.Pool(processes=args.num_cpus) as pool:
            pool.map(run_on_one_file_parallel, func_args)
    else:
        for func_arg in tqdm(func_args):
            run_on_one_file_parallel(func_arg)
